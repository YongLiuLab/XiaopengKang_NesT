    ...
    with open(os.path.join(_dir, 'log.txt'), 'w') as f:
        ...
        for epoch in range(train_setting.epoches):
            ...
            if epoch % train_setting.save_every_n == train_setting.save_every_n-1:
                test, acc, *_ = model_utils.test_model(model, valid_loader, criterion, phase='test', f=f)
                torch.save(model.state_dict(), os.path.join(_dir, f'Model_acc{acc:.2f}_epoch{epoch}.pt'))
        ...
        df.to_csv(os.path.join(_dir, f'Train_Report.csv'))

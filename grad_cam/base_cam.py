...
class BaseCAM:
    ...
    def get_target_width_height_depth(self, input_tensor):
        # ADD depth
        width, height, depth = input_tensor.size(-3), input_tensor.size(-2), input_tensor.size(-1)
    ...
    def scale_cam_image(self, cam, target_size=None):
        ...
        for img in cam:
            ...
            if target_size is not None:
                a, b, c = img.shape
                x, y, z = target_size
                img = ndimage.zoom(img, (x/a, y/b, z/c))
            result.append(img)

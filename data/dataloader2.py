...
class ArguDataset(tio.SubjectsDataset):
    ...
    def __getitem__(self, index):
        ...
        baseline_info = subject.sub.info.loc['baseline']
        age = baseline_info['age']
        gender = baseline_info['male']
        other_features = [age, gender]
        other_features = torch.FloatTensor(other_features)

        subject_name = f'{center}_{name}'
        return datas, label, subject_name, other_features